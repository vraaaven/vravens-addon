...
public class ShatteredCrescentRenderer extends EntityRenderer<ShatteredCrescentStrike> {

    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(VravenAddon.MOD_ID, "textures/entity/spells/shattered_crescent.png");

    public ShatteredCrescentRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(ShatteredCrescentStrike entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
        ...
        float maxAge = 12.0f;
        float progress = ageInTicks / maxAge;
        if (progress > 1.0f) progress = 1.0f;

        float width = 2.5f + (progress * 3.5f);
        float height = 5.0f + (progress * 7.0f);
        float alphaMult = 1.0f - (progress * progress);
        ...
        if (alpha > 0) {
            ...
            int g = 30;
            int b = 245;
            renderBladeLayers(poseStack, consumer, width, height, alpha, r, g, b);
        }

        poseStack.popPose();
    }

    private void renderBladeLayers(PoseStack poseStack, VertexConsumer consumer, float width, float height, int alpha, int r, int g, int b) {
        float offset = 0.005f;
    ...
    @Override
    public ResourceLocation getTextureLocation(ShatteredCrescentStrike entity) { return TEXTURE; }
}