...
public class ThunderClapSlashEffectEntity extends Entity implements IEntityWithComplexSpawn {
    public static final int LIFETIME = 10;
    private float yRot;
    private float randomRotOffset;

    public ThunderClapSlashEffectEntity(EntityType<?> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    }

    public ThunderClapSlashEffectEntity(Level level, Entity target, float yRot) {
        this(EntityRegistry.THUNDER_CLAP_SLASH_EFFECT.get(), level);
        this.yRot = yRot;
