...
public class RedEmbersParticle extends TextureSheetParticle {
    private final SpriteSet sprites;
    private final boolean mirrored;

    public RedEmbersParticle(ClientLevel level, double xCoord, double yCoord, double zCoord, SpriteSet spriteSet, double xd, double yd, double zd) {
    ...
    public int getLightColor(float p_107564_) {
        return 0xF000F0; // Максимальное свечение
    }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType particleType, ClientLevel level, double x, double y, double z, double dx, double dy, double dz) {
            return new RedEmbersParticle(level, x, y, z, this.sprites, dx, dy, dz);
        }
    }
}