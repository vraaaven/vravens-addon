...
public class DarkSlashParticle extends TextureSheetParticle {
    private final SpriteSet sprites;
    private final Vec3 forward, up;
    private final Vector3f[] localVertices;

    DarkSlashParticle(ClientLevel pLevel, double pX, double pY, double pZ, SpriteSet spriteSet, double xd, double yd, double zd, DarkSlashParticleOptions options) {
    ...
    private void createEmberTrail() {
        ...
        for (int i = 1; i < particleCount - 1; i++) {
            ...
            if (random.nextFloat() < .5f) {
                level.addParticle(ParticleRegistry.DARK_EMBERS.get(), x + pos.x, y + pos.y, z + pos.z, motion.x * 1.5, motion.y * 1.5, motion.z * 1.5);
            }
        }
    }
    ...
    private void makeCornerVertex(VertexConsumer pConsumer, Vector3f pVec3f, float pU, float pV, int pLight) {
        pConsumer.addVertex(pVec3f.x(), pVec3f.y(), pVec3f.z()).setUv(pU, pV).setColor(this.rCol, this.gCol, this.bCol, this.alpha).setLight(pLight);
    }
    ...
    protected int getLightColor(float pPartialTick) {
        ...
        return 0; // Полная темнота
    }

    @OnlyIn(Dist.CLIENT)
    public static class Provider implements ParticleProvider<DarkSlashParticleOptions> {
        ...
        public Particle createParticle(@NotNull DarkSlashParticleOptions options, @NotNull ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
            DarkSlashParticle particle = new DarkSlashParticle(pLevel, pX, pY, pZ, sprite, pXSpeed, pYSpeed, pZSpeed, options);
