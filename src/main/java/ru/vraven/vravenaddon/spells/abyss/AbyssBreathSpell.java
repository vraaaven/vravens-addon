...
import ru.vraven.vravenaddon.entity.spells.AbyssBreathProjectile;

import java.util.List;
import java.util.Optional;

public class AbyssBreathSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(VravenAddon.MOD_ID, "abyss_breath");
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        if (playerMagicData.isCasting() && playerMagicData.getCastingSpellId().equals(this.getSpellId())
            ...
            AbyssBreathProjectile projectile = new AbyssBreathProjectile(world, entity);
            projectile.setPos(entity.position().add(0, entity.getEyeHeight() * .7, 0));
            projectile.setDamage(getDamage(spellLevel, entity));
            world.addFreshEntity(projectile);

            playerMagicData.setAdditionalCastData(new EntityCastData(projectile));
        }
        super.onCast(world, spellLevel, entity, castSource, playerMagicData);
    }

    public float getDamage(int spellLevel, LivingEntity caster) {
        return 2 + getSpellPower(spellLevel, caster) * 0.8f;
    }
